...
//#endregion
//#region node_modules/@tonconnect/isomorphic-eventsource/index.mjs
var import_eventsource = /* @__PURE__ */ __toESM((/* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var maxBufferAheadAllocation = 262144;
	...
	**/
	function EventSource(url, eventSourceInitDict) {
		...
		function connect() {
			...
			if (eventSourceInitDict && eventSourceInitDict.createConnection !== void 0) options.createConnection = eventSourceInitDict.createConnection;
			...
			if (eventSourceInitDict && eventSourceInitDict.https) for (var optName in eventSourceInitDict.https) {
				...
				if (option !== void 0) options[optName] = option;
			}
			if (eventSourceInitDict && eventSourceInitDict.withCredentials !== void 0) options.withCredentials = eventSourceInitDict.withCredentials;
			req = (isSecure ? https : http).request(options, function(res) {
				...
				if (res.statusCode === 301 || res.statusCode === 302 || res.statusCode === 307) {
					...
					hasNewOrigin = new URL(url).origin !== new URL(location).origin;
				...
				res.on("data", function(chunk) {
					if (!buf) {
						buf = chunk;
						if (hasBom(buf)) buf = buf.slice(bom.length);
						bytesUsed = buf.length;
					} else {
						if (chunk.length > buf.length - bytesUsed) {
							newBufferSize = buf.length * 2 + chunk.length;
							if (newBufferSize > maxBufferAheadAllocation) newBufferSize = buf.length + chunk.length + maxBufferAheadAllocation;
	...
	*/
	function removeUnsafeHeaders(headers) {
		...
		for (var key in headers) {
			if (reUnsafeHeader.test(key)) continue;
