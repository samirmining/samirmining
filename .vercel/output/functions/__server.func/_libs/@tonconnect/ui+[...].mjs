...
import { c as SessionCrypto, n as CHAIN } from "./protocol+[...].mjs";
import { A as createRequestVersionEvent, B as isConnectUrl, C as createConnectionRestoringErrorEvent, D as createDataSignedEvent, E as createDataSentForSignatureEvent, F as createTransactionSigningFailedEvent, G as isWalletInfoCurrentlyInjected, H as isTelegramUrl, I as createVersionInfo, J as toUserFriendlyAddress, K as isWalletInfoInjectable, L as createWalletModalOpenedEvent, M as createSelectedWalletEvent, N as createTransactionSentForSignatureEvent, O as createDataSigningFailedEvent, P as createTransactionSignedEvent, R as decodeTelegramUrlParameters, S as createConnectionRestoringCompletedEvent, T as createConnectionStartedEvent, U as isWalletConnectInitialized, V as isQaModeEnabled, W as isWalletInfoCurrentlyEmbedded, _ as WalletsListManager, a as TonConnect, b as createConnectionCompletedEvent, c as UnknownAppError, d as WalletAlreadyConnectedError, f as WalletMissingRequiredFeaturesError, g as WalletWrongNetworkError, h as WalletNotSupportFeatureError, i as ParseHexError, j as createResponseVersionEvent, k as createDisconnectionEvent, l as UnknownError, m as WalletNotInjectedError, n as BrowserEventDispatcher, o as TonConnectError, p as WalletNotConnectedError, q as isWalletInfoRemote, r as FetchWalletsError, s as UUIDv7, t as BadRequestError, u as UserRejectsError, v as WrongAddressError, w as createConnectionRestoringStartedEvent, x as createConnectionErrorEvent, y as checkRequiredWalletFeatures, z as encodeTelegramUrlParameters } from "../tonconnect__sdk.mjs";
...
//#endregion
//#region node_modules/@tonconnect/ui/lib/index.mjs
...
var import_ua_parser = /* @__PURE__ */ __toESM(require_ua_parser(), 1);
...
function createProvider(id, options) {
	return function provider(props) {
		...
		createRenderEffect(() => res = untrack(() => {
			Owner.context = __spreadProps(__spreadValues({}, Owner.context), { [id]: props.value });
...
function Show(props) {
	...
	const condition = keyed ? conditionValue : createMemo(conditionValue, void 0, { equals: (a2, b) => !a2 === !b });
	return createMemo(() => {
		const c2 = condition();
		if (c2) {
			const child = props.children;
			return typeof child === "function" && child.length > 0 ? untrack(() => child(keyed ? c2 : () => {
...
function Switch(props) {
	...
	const switchFunc = createMemo(() => {
		...
		for (let i2 = 0; i2 < mps.length; i2++) {
			...
			const condition = mp.keyed ? conditionValue : createMemo(conditionValue, void 0, { equals: (a2, b) => !a2 === !b });
...
function insertExpression(parent, value, current, marker, unwrapArray) {
	...
	} else if (value == null || t2 === "boolean") current = cleanChildren(parent, current, marker);
	else if (t2 === "function") {
		createRenderEffect(() => {
			let v = value();
...
function normalizeIncomingArray(normalized, array, current, unwrap2) {
	...
	for (let i2 = 0, len = array.length; i2 < len; i2++) {
		...
		else if ((t2 = typeof item) === "object" && item.nodeType) normalized.push(item);
...
function makeStyled(tag) {
	...
	return (...args) => {
		const Styled = (props) => {
			const withTheme = mergeProps(props, { theme: useContext(ThemeContext) });
			const [local, newProps] = splitProps(mergeProps(withTheme, { get class() {
				const pClass = withTheme.class, append = "class" in withTheme && /^go[0-9]+/.test(pClass);
				return [pClass, u.apply({
					target: _ctx.target,
					o: append,
					p: withTheme,
					g: _ctx.g
...
var styled = new Proxy(makeStyled, { get(target, tag) {
	return target(tag);
} });
...
var GlobalStyles = () => {
	...
	return createComponent(createGlobalStyles`
    ${globalStylesTag} * {
        margin: 0;
        padding: 0;
...
function openDeeplinkWithFallback(href, fallback) {
	const doFallback = () => {
		if (isBrowser("safari") || isOS("android") && isBrowser("firefox") || isOS("windows")) return;
		fallback();
	};
...
function enableScroll() {
	if (!document.body.classList.contains(disableScrollClass)) return;
...
function defineStylesRoot() {
	if (!customElements.get("tc-root")) customElements.define(globalStylesTag, class TcRootElement extends HTMLElement {});
}
...
function getUserAgent() {
	...
	switch (true) {
		...
		case osName === "linux":
			os = "linux";
			break;
		case osName == null ? void 0 : osName.includes("windows"): os = "windows";
	}
	...
	switch (true) {
		...
		case browserName == null ? void 0 : browserName.includes("safari"):
			browser = "safari";
			break;
		case browserName == null ? void 0 : browserName.includes("opera"): browser = "opera";
	}
	return {
		os,
		browser
...
function toDeeplink(universalLink, deeplink) {
	return deeplink + new URL(universalLink).search;
}
...
var [lastOpenedLink, setLastOpenedLink] = createSignal({ link: "" });
var i18nDictionary = {
	en: parseDictionary({
		common: {
			"close": "Close",
		...
		notifications: {
			...
			"dataSigned": { "header": "Data signed" },
			"signDataCanceled": { "header": "Sign data canceled" }
		},
		walletItem: {
			"walletOn": "Wallet in",
			"recent": "Recent",
	...
	ru: parseDictionary({
		...
		notifications: {
			...
			"dataSigned": { "header": "Данные подписаны" },
			"signDataCanceled": { "header": "Подпись данных отменена" }
		},
		walletItem: {
			"walletOn": "Wallet в",
			"recent": "Недавний",
...
function getNodes(target, symbol) {
	...
	if (!nodes) Object.defineProperty(target, symbol, { value: nodes = /* @__PURE__ */ Object.create(null) });
	return nodes;
}
...
function updatePath(current, path, traversed = []) {
	...
	if (path.length > 1) {
		...
		if (Array.isArray(part)) {
			...
			for (let i2 = from; i2 <= to; i2 += by) updatePath(current, [i2].concat(path), traversed);
			return;
...
var initParams = {};
try {
	initParams = urlParseHashParams(location.hash.toString());
} catch (e2) {}
...
function updateFromStoredParams(key) {
	try {
		...
		if (storedParams) {
			for (const key2 in storedParams) if (typeof initParams[key2] === "undefined") initParams[key2] = storedParams[key2];
		}
	} catch (e2) {}
}
function updateFromStoredRawParams(key) {
	...
	try {
		...
		try {
			...
			if (typeof parsedJsonRaw === "string") raw = parsedJsonRaw;
			else return;
		} catch (e2) {}
		const storedParams = urlParseQueryString(raw);
		for (const paramKey in storedParams) {
			const value = storedParams[paramKey];
			if (value != null && typeof initParams[paramKey] === "undefined") initParams[paramKey] = value;
		}
	} catch (e2) {}
}
...
try {
	if (Object.entries(initParams).length > 0) sessionStorageSet(LAUNCH_PARAMS_STORAGE_KEY, JSON.stringify(initParams));
} catch (e2) {}
...
try {
	if (initDataRaw) {
		let userRaw = urlParseQueryString(initDataRaw).user;
		if (userRaw) {
			let user = JSON.parse(userRaw);
			if (typeof user.id === "number" && typeof user.is_premium === "boolean") telegramUser = {
				id: user.id,
...
function useDataAttributes(props) {
	const [dataAttrs] = splitProps(props, untrack(() => Object.keys(props).filter((key) => key.startsWith("data-"))));
	return dataAttrs;
}
var Button = (props) => {
...
function createSwitchTransition(source, options) {
	...
	const triggerTransitions = options.mode === "out-in" ? ((prev) => isExiting || exitTransition2(prev, enterTransition2)) : options.mode === "in-out" ? ((prev) => enterTransition2(() => exitTransition2(prev))) : ((prev) => {
...
var ROUTE_STATE = { [ROUTE_STATE_KEY]: true };
...
var ArrowIcon = (props) => {
	...
	return createComponent(styled("svg")`
...
var _AnimationNoop = class _AnimationNoop {
	constructor() {
		this.currentTime = 0;
		this.playbackRate = 1;
		this.startTime = null;
	...
	(function(module, exports) {
		var qrcode2 = function() {
			var qrcode3 = function(typeNumber, errorCorrectionLevel) {
					...
					for (var col = _moduleCount - 1; col > 0; col -= 2) {
						...
						while (true) {
							for (var c2 = 0; c2 < 2; c2 += 1) if (_modules[row][col - c2] == null) {
								...
								if (maskFunc(row, col - c2)) dark = !dark;
				...
				var createBytes = function(buffer, rsBlocks) {
					...
					for (var r = 0; r < rsBlocks.length; r += 1) {
						...
						var modPoly = qrPolynomial(dcdata[r], rsPoly.getLength() - 1).mod(rsPoly);
				...
				_this.createSvgTag = function(cellSize, margin, alt, title) {
					...
					var c2, mc, r, mr, qrSvg = "", rect = "l" + cellSize + ",0 0," + cellSize + " -" + cellSize + ",0 0,-" + cellSize + "z ";
			...
			function qrPolynomial(num, shift2) {
				if (typeof num.length == "undefined") throw num.length + "/" + shift2;
				var _num = function() {
					var offset = 0;
...
var qrcode = /* @__PURE__ */ getDefaultExportFromCjs(requireQrcode());
...
var QRCode = (props) => {
	...
	createEffect(() => {
		...
		if (imageRef) {
			...
			const imgOffset = toPx(Math.ceil((qrSize - imgSize) / 8) * cellSize);
		...
		get children() {
			return [
				memo(() => memo(() => typeof props.icon === "string")() ? createComponent(ImageStyled$1, {
					...
					get badge() {
						return memo(() => !!props.badgeUrl)() && createComponent(BadgeStyled, { get src() {
...
var _tmpl$2$2 = /* @__PURE__ */ template$1(`<svg xmlns=http://www.w3.org/2000/svg width=44 height=44 viewBox="0 0 44 44"fill=none><path fill-rule=evenodd clip-rule=evenodd d="M22 2.99951C11.5066 2.99951 3 11.5061 3 21.9995C3 32.4929 11.5066 40.9995 22 40.9995C22.8284 40.9995 23.5 41.6711 23.5 42.4995C23.5 43.3279 22.8284 43.9995 22 43.9995C9.84974 43.9995 0 34.1498 0 21.9995C0 9.84925 9.84974 -0.000488281 22 -0.000488281C34.1503 -0.000488281 44 9.84925 44 21.9995C44 22.8279 43.3284 23.4995 42.5 23.4995C41.6716 23.4995 41 22.8279 41 21.9995C41 11.5061 32.4934 2.99951 22 2.99951Z">`);
var _tmpl$3 = /* @__PURE__ */ template$1(`<svg width=72 height=72 viewBox="0 0 72 72"fill=none xmlns=http://www.w3.org/2000/svg><path d="M24 56.7846C35.479 63.412 50.1572 59.479 56.7846 47.9999C63.412 36.5209 59.479 21.8427 48 15.2153C36.521 8.58791 21.8428 12.5209 15.2154 23.9999"stroke-width=4 stroke-linecap=round stroke-linejoin=round>`);
...
var TonConnectBrand = () => {
	...
	return (() => {
		var _el$ = _tmpl$$m(), _el$4 = _el$.firstChild.nextSibling.nextSibling;
...
var RetryIcon = (props) => {
	...
	return (() => {
		var _el$ = _tmpl$$h(), _el$3 = _el$.firstChild.firstChild;
...
var WalletIcon = (props) => {
	...
	const fill = () => props.fill || theme.colors.accent;
	return (() => {
		var _el$ = _tmpl$$f(), _el$3 = _el$.firstChild.firstChild;
		createRenderEffect(() => setAttribute(_el$3, "fill", fill()));
		return _el$;
	})();
};
...
var WalletLabeledItem = (props) => {
	...
	const walletsSecondLine = () => {
		if (props.wallet.appName === AT_WALLET_APP_NAME) return;
		...
		if (isWalletUi(props.wallet) && isWalletInfoCurrentlyInjected(props.wallet)) return t2("walletItem.installed", {}, "Installed");
	};
...
var AccountButtonDropdown = (props) => {
	...
	const onCopy = () => __async(null, null, function* () {
		yield copyToClipboard(toUserFriendlyAddress(tonConnectUi.account.address, tonConnectUi.account.chain === CHAIN.TESTNET));
...
var ErrorTransactionNotification = (props) => {
	return createComponent(Notification, {
		...
		get icon() {
			return createComponent(ErrorIconStyled$4, { size: "xs" });
		},
		get ["class"]() {
			return props.class;
		},
...
var NotificationClass = u`
    ...
    margin-bottom: 12px;
`;
var defaultConfig = { timeout: 4500 };
...
function useOpenedNotifications(config) {
	...
	createEffect(on(action, (action2) => {
		...
		if (isConfirmTransactionAction || isConfirmSignDataAction) return;
...
var ErrorSignDataNotification = (props) => {
	return createComponent(Notification, {
		...
		get icon() {
			return createComponent(ErrorIconStyled$3, { size: "xs" });
		},
		get ["class"]() {
			return props.class;
		},
					...
					children: (openedNotification) => createComponent(Switch, { get children() {
						return [
							...
							createComponent(Match, {
								...
								get children() {
									return createComponent(SuccessSignDataNotification, { "class": NotificationClass });
								}
							}),
							createComponent(Match, {
								get when() {
								...
								get children() {
									return createComponent(ErrorSignDataNotification, { "class": NotificationClass });
								}
							}),
							createComponent(Match, {
								get when() {
	...
	return {
		...
		fn(state) {
			return __async(this, null, function* () {
				...
				const _a3 = evaluate(options, state), { mainAxis: checkMainAxis = true, crossAxis: checkCrossAxis = false, limiter = { fn: (_ref) => {
					let { x: x2, y: y2 } = _ref;
					return {
						x: x2,
						y: y2
				...
				return __spreadProps(__spreadValues({}, limitedCoords), { data: {
					x: limitedCoords.x - x,
					y: limitedCoords.y - y,
					enabled: {
...
function observeMove(element, onMove) {
	...
	function refresh(skip, threshold) {
		...
		try {
			io = new IntersectionObserver(handleObserve, __spreadProps(__spreadValues({}, options), { root: root.ownerDocument }));
		} catch (_e2) {
...
var _tmpl$2$1 = /* @__PURE__ */ template$1(`<svg xmlns=http://www.w3.org/2000/svg width=28 height=28 viewBox="0 0 28 28"fill=none><circle cx=14 cy=14 r=12></circle><circle cx=14 cy=18.9 r=1.4></circle><path d="M12.6658 8.89845C12.63 8.13698 13.2377 7.5 14 7.5C14.7623 7.5 15.37 8.13698 15.3342 8.89845L15.047 15.0013C15.0207 15.5604 14.5597 16.0002 14 16.0002C13.4403 16.0002 12.9793 15.5604 12.953 15.0013L12.6658 8.89845Z">`);
var ExclamationIcon = (props) => {
	const theme = useTheme();
	const size = () => props.size;
	const fill = () => props.fill || theme.colors.icon.error;
	return memo(() => memo(() => size() === "16")() ? (() => {
		var _el$ = _tmpl$$3(), _el$2 = _el$.firstChild, _el$3 = _el$2.firstChild, _el$4 = _el$3.nextSibling, _el$5 = _el$4.nextSibling, _el$8 = _el$2.nextSibling.firstChild.firstChild;
		createRenderEffect((_p$) => {
...
var AllWalletsListModal = (props) => {
	...
	const connectWalletConnect = () => {
		connector.connect({ type: "wallet-connect" }, (additionalRequest == null ? void 0 : additionalRequest.state) === "ready" ? additionalRequest.value : void 0);
	};
...
function redirectToTelegram(universalLink, options) {
	...
	if (isInTelegramBrowser()) {
		...
		if (isTmaPlatform("ios", "android", "macos", "tdesktop", "web")) {
			options.returnStrategy = "back";
			sendOpenTelegramLink(addReturnStrategy(directLinkUrl.toString(), options.returnStrategy));
		...
		if ((isChrome || isFirefox) && !options.forceRedirect) {
			const linkWithStrategy = addReturnStrategy(directLinkUrl.toString(), options.returnStrategy);
			openDeeplinkWithFallback(convertToTGDeepLink(linkWithStrategy), () => openLinkBlank(linkWithStrategy));
		} else openLinkBlank(addReturnStrategy(directLinkUrl.toString(), options.returnStrategy));
		...
		const linkWithStrategy = addReturnStrategy(directLinkUrl.toString(), options.returnStrategy);
		if (useDeepLink) openDeeplinkWithFallback(convertToTGDeepLink(linkWithStrategy), () => openLinkBlank(linkWithStrategy));
		...
		else openDeeplinkWithFallback(convertToTGDeepLink(linkWithStrategy), () => openLinkBlank(linkWithStrategy));
	} else openLinkBlank(addReturnStrategy(directLinkUrl.toString(), options));
}
function redirectToWallet(universalLink, deepLink, options, setOpenMethod) {
	...
	if (isInTelegramBrowser()) {
		...
		if (isTmaPlatform("ios", "android")) {
			...
			if (options.returnStrategy === "back") options.returnStrategy = "tg://resolve";
			const linkWithStrategy = addReturnStrategy(universalLink, options.returnStrategy);
			if (!!deepLink && !options.forceRedirect) {
...
function enrichUniversalLink(universalLink, params) {
	var _a2, _b2;
	if (!isTelegramUrl(universalLink)) return addQueryParametersIfNotPresented(universalLink, {
		id: params.sessionId,
		trace_id: params.traceId
	});
	const directLink = convertToTGDirectLink(universalLink);
	const directLinkUrl = new URL(directLink);
	let searchParams = buildSearchParams(decodeTelegramUrlParameters((_a2 = directLinkUrl.searchParams.get("startapp")) != null ? _a2 : ""));
	...
	const unorderedParams = Array.from(searchParams.entries()).filter(([key]) => !orderedKeys.includes(key));
	const startApp = buildQueryParams(__spreadProps(__spreadValues({
		tonconnect: "",
		v: "2",
		id: sessionId
	}, Object.fromEntries(unorderedParams)), { trace_id: traceId })).replace("=&", "&");
...
function addQueryParametersIfNotPresented(url, params) {
	const parsed = new URL(url);
	for (const [key, value] of Object.entries(params)) if (value !== void 0 && value !== null) parsed.searchParams.set(key, value);
	return parsed.toString();
}
...
function buildQueryParams(params) {
	...
	for (const [key, value] of Object.entries(params)) if (value !== void 0 && value !== null) searchParams.append(key, value);
	return searchParams.toString();
}
function convertToTGDeepLink(directLink) {
	const parsed = new URL(directLink);
	const [, domain, appname] = parsed.pathname.split("/");
	return `tg://resolve?domain=${domain}&appname=${appname}&startapp=${parsed.searchParams.get("startapp")}`;
}
var DesktopConnectionModal = (props) => {
	...
	const connector = useContext(ConnectorContext);
	onCleanup(connector.onStatusChange(() => {}, (error) => {
	...
	const generateUniversalLink = () => {
		...
		try {
			...
			}, props.additionalRequest, { traceId: (_a2 = props.walletsModalState) == null ? void 0 : _a2.traceId });
	...
	const onClickExtension = () => {
		...
		if (isWalletInfoCurrentlyInjected(props.wallet)) {
			...
			connector.connect({ jsBridgeKey: props.wallet.jsBridgeKey }, props.additionalRequest, { traceId: (_a2 = props.walletsModalState) == null ? void 0 : _a2.traceId });
		}
	};
	...
	return createComponent(DesktopConnectionModalStyled, {
		...
		get children() {
			return [
				...
				createComponent(H1Styled$8, { get children() {
					return props.wallet.name;
				} }),
				createComponent(Show, {
					get when() {
						...
						return createComponent(Switch, { get children() {
							return [
								createComponent(Match, {
									...
									get children() {
										return [
											createComponent(ErrorIconStyled$2, { size: "s" }),
											...
											createComponent(ButtonsContainerStyled$2, { get children() {
												return [
													createComponent(Show, {
														get when() {
															return connectionErrored() !== "not-supported";
...
var ButtonsBlock = styled.div`
    ...
    gap: 8px;
`;
var LINKS = { GET_A_WALLET: "https://ton.org/wallets?filters[wallet_features][slug][$in]=dapp-auth&pagination[limit]=-1" };
var InfoModal = (props) => {
...
var MobileConnectionQR = (props) => {
	return [
		createComponent(H1Styled$5, { get children() {
...
var MobileConnectionModal = (props) => {
	...
	const universalLink = createMemo(() => {
		...
		return connector.connect({
			universalLink: props.wallet.universalLink,
			bridgeUrl: props.wallet.bridgeUrl
		}, props.additionalRequest, { traceId: (_a2 = props.walletModalState) == null ? void 0 : _a2.traceId });
	});
	const onClickTelegram = () => {
...
var _tmpl$$1 = /* @__PURE__ */ template$1(`<li>`);
var _tmpl$2 = /* @__PURE__ */ template$1(`<div data-tc-wallets-modal-universal-mobile=true>`);
var MobileUniversalModal = (props) => {
	...
	const getUniversalLink = () => {
		if (!universalLink()) setUniversalLink(connector.connect(walletsBridges(), props.additionalRequest, { traceId: props.walletModalState.traceId }));
		return universalLink();
	};
	...
	const onSelectTelegram = () => {
		...
		}, props.additionalRequest, { traceId: props.walletModalState.traceId });
		const forceRedirect = !firstClick();
		setFirstClick(false);
...
var DesktopUniversalModal = (props) => {
	...
	const walletsBridges = createMemo(() => getUniqueBridges(props.walletsList), null, { equals: bridgesIsEqual });
	setLastSelectedWalletInfo({ openMethod: "qrcode" });
	const request = createMemo(() => connector.connect(walletsBridges(), props.additionalRequest, { traceId: props.walletModalState.traceId }));
	...
	return createComponent(RestoreInfoModalStyled, {
		...
		get children() {
			return [
				...
				createComponent(ScrollContainer, { get children() {
					return [
						createComponent(StepBlock, { get children() {
							return [
								createComponent(CircleNumber, { children: "1" }),
						...
						createComponent(StepBlock, { get children() {
							return [
								createComponent(CircleNumber, { children: "2" }),
								createComponent(H3Styled, {
						...
						createComponent(StepBlock, { get children() {
							return [
								createComponent(CircleNumber, { children: "3" }),
								createComponent(H3Styled, {
...
var DesktopFeatureNotSupportModal = (props) => {
	...
	const currentWalletName = createMemo(() => {
		var _a2, _b2, _c2, _d2;
		if (((_b2 = (_a2 = currentWalletUI()) == null ? void 0 : _a2.appName) != null ? _b2 : props.currentWallet.device.appName) === AT_WALLET_APP_NAME) return "Wallet in Telegram";
	...
	const currentWalletVersionNotSupported = createMemo(() => {
		...
		if (!(currentWalletUIVar == null ? void 0 : currentWalletUIVar.features)) return false;
	...
	return createComponent(DesktopFeatureNotSupportModalStyled, { get children() {
		return createComponent(Switch, { get children() {
			return [
				createComponent(Match, {
					get when() {
					...
					get children() {
						return createComponent(RestoreInfoModal, { onBackClick: () => setInfoModalOpen(false) });
					}
				}),
				createComponent(Match, {
					get when() {
					...
					get children() {
						return [createComponent(Spacer, {}), createComponent(DesktopFeatureNotSupportModalStyled, { get children() {
							return [
								...
								createComponent(DescriptionStyled, {
									...
									get translationValues() {
										return { name: selectedWallet().name };
									},
									get children() {
										return [
				...
				createComponent(Match, {
					...
					get children() {
						return [
							createComponent(H1Styled$2, { get children() {
					...
					get children() {
						return [
							...
							createComponent(TitleStyled, {
								...
								get children() {
									return [memo(() => currentWalletName()), " doesn’t support the requested\xA0action"];
								}
							}),
							createComponent(Spacer, {})
						];
...
var WalletsModal = () => {
	...
	const walletsList = createMemo(() => {
		...
		return walletsList2.map((wallet) => {
			var _a3;
			return __spreadProps(__spreadValues({}, wallet), { isSupportRequiredFeatures: walletsFeaturesRequested ? checkRequiredWalletFeatures((_a3 = wallet.features) != null ? _a3 : [], walletsFeaturesRequested) : true });
		});
	});
	...
	const additionalRequest = createMemo(() => {
		var _a2;
		if (additionalRequestLoading()) return;
...
var ActionModal = (props) => {
	...
	const onOpenWallet = () => {
		...
		if (isTelegramUrl(universalLink)) redirectToTelegram(linkWithSessionId, {
			returnStrategy,
			twaReturnUrl: "twaReturnUrl" in currentAction ? currentAction.twaReturnUrl : appState.twaReturnUrl,
			forceRedirect
		});
		else redirectToWallet(linkWithSessionId, deepLink, {
			returnStrategy,
			forceRedirect
		}, () => {});
	};
			...
			return createComponent(Switch, { get children() {
				return [
					...
					createComponent(Match, {
						...
						get children() {
							return createComponent(DataSignedModal, { onClose: () => setAction(null) });
						}
					}),
					createComponent(Match, {
						get when() {
						...
						get children() {
							return createComponent(SignDataCanceledModal, { onClose: () => setAction(null) });
						}
					}),
					createComponent(Match, {
						get when() {
						...
						get children() {
							return createComponent(ConfirmSignDataModal, { onClose: () => setAction(null) });
						}
					})
				];
			} });
...
var SingleWalletModal = () => {
	...
	onCleanup(connector.onStatusChange((wallet) => {
		if (wallet) onClose("wallet-selected");
	}));
	onCleanup(() => {
		setInfoTab(false);
		...
		createEffect(on(lastSelectedWalletInfo, (selectedWallet, previousWallet) => __async(this, null, function* () {
			try {
				...
				if (selectedWallet && "appName" in selectedWallet) {
					if (previousWallet && "appName" in previousWallet && previousWallet.appName === selectedWallet.appName) return;
	...
	*/
	open(options) {
		return __async(this, null, function* () {
			...
			const embeddedWallet = (yield this.connector.getWallets()).find(isWalletInfoCurrentlyEmbedded);
			if (embeddedWallet) return this.connectEmbeddedWallet(embeddedWallet, { traceId });
			else return this.openWalletsModal({ traceId });
		});
	}
	/**
	...
	*/
	connectEmbeddedWallet(embeddedWallet, options) {
		const connect = (parameters) => {
			...
			this.connector.connect({ jsBridgeKey: embeddedWallet.jsBridgeKey }, parameters, { traceId: options.traceId });
		};
		const additionalRequest = appState.connectRequestParameters;
...
var SingleWalletModalManager = class {
	...
	*/
	open(wallet) {
		return __async(this, null, function* () {
			this.tracker.trackConnectionStarted();
			const walletsList = applyWalletsListConfiguration(yield this.connector.getWallets(), appState.walletsListConfiguration);
			...
			const externalWallet = walletsList.filter(isWalletInfoRemote).find((walletInfo) => eqWalletName(walletInfo, wallet));
			if (!!externalWallet) return this.openSingleWalletModal(externalWallet);
	...
	setRequestVersionHandler() {
		return __async(this, null, function* () {
			yield this.eventDispatcher.addEventListener("ton-connect-ui-request-version", () => __async(this, null, function* () {
				var _a2;
				let response = createResponseVersionEvent(this.tonConnectUiVersion);
				try {
					...
					response = __spreadProps(__spreadValues({}, response), {
						...
						os: ua.os,
						browser: ua.browser,
						rawBrowser: (_a2 = new import_ua_parser.default().getBrowser().name) != null ? _a2 : null,
						mobile: isMobileUserAgent()
					});
				} catch (e2) {}
				yield this.eventDispatcher.dispatchEvent("ton-connect-ui-response-version", response);
			}));
		});
	}
	/**
...
var TonConnectUI = class {
	...
	*/
	set uiOptions(options) {
		...
		if ("walletsRequiredFeatures" in options) this._walletsRequiredFeatures = options.walletsRequiredFeatures;
		if ("walletsPreferredFeatures" in options) this._walletsPreferredFeatures = options.walletsPreferredFeatures;
	...
	*/
	connectWallet(options) {
		return __async(this, null, function* () {
			var _a2;
			const traceId = (_a2 = options == null ? void 0 : options.traceId) != null ? _a2 : UUIDv7();
			const embeddedWallet = (yield this.getWallets()).find(isWalletInfoCurrentlyEmbedded);
			if (embeddedWallet) return yield this.connectEmbeddedWallet(embeddedWallet, { traceId });
			else return yield this.connectExternalWallet({ traceId });
		});
	}
	/**
	...
	sendTransaction(tx, options) {
		return __async(this, null, function* () {
			...
			const onRequestSent = () => {
				...
				(_a3 = options == null ? void 0 : options.onRequestSent) == null || _a3.call(options, redirectToWallet2);
			};
	...
	*/
			...
			try {
				...
				if (storage) {
					...
					if (stored) {
						...
						if (connection.type === "http" && connection.sessionCrypto) return new SessionCrypto(connection.sessionCrypto).sessionId;
						else if (connection.type === "http" && ((_b2 = connection.session) == null ? void 0 : _b2.sessionKeyPair)) return new SessionCrypto(connection.session.sessionKeyPair).sessionId;
					}
				}
			} catch (e2) {}
			return null;
	...
	redirectAfterRequestSent({ returnStrategy, twaReturnUrl, forceRedirect, sessionId, traceId }) {
		if (this.walletInfo && "universalLink" in this.walletInfo && (this.walletInfo.openMethod === "universal-link" || this.walletInfo.openMethod === "custom-deeplink")) {
			...
			if (isTelegramUrl(this.walletInfo.universalLink)) redirectToTelegram(linkWithSessionId, {
				returnStrategy,
				twaReturnUrl: twaReturnUrl || appState.twaReturnUrl,
				forceRedirect: forceRedirect || false
			});
			else redirectToWallet(linkWithSessionId, this.walletInfo.deepLink, {
				returnStrategy,
				forceRedirect: forceRedirect || false
			}, () => {});
		}
	...
	getSelectedWalletInfo(wallet) {
		return __async(this, null, function* () {
			...
			if (!("name" in lastSelectedWalletInfo2)) {
				if (wallet.device.appName === WALLET_CONNECT_APP_NAME) return {
					type: "wallet-connect",
					name: WALLET_CONNECT_WALLET_NAME,
					appName: WALLET_CONNECT_APP_NAME,
					imageUrl: IMG.WALLET_CONNECT,
				...
				const walletInfo = applyWalletsListConfiguration(yield this.walletsList, appState.walletsListConfiguration).find((item) => eqWalletName(item, wallet.device.appName));
