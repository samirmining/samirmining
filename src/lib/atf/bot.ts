import { isAdminId, isOwnerId } from "./admins";
import { DEFAULT_TASK_BROADCAST, REACT_TASK_ID } from "./constants";
import { newId } from "./crypto";
import { round4 } from "./mining";
import { rateLimit } from "./security";
import { getStore, webappUrl } from "./store";
import type { Task, User, WelcomeButton } from "./types";
...
type Flow =
  | { kind: "announce"; step: "text" | "btn" | "url"; text?: string; btn?: string; photo?: string }
  | { kind: "announcepic"; step: "photo" | "text" | "btn" | "url"; text?: string; btn?: string; photo?: string }
  | { kind: "addtask"; step: "name" | "desc" | "link" | "reward"; name?: string; desc?: string; link?: string }
  | { kind: "edittask"; step: "pick" | "field" | "value"; taskId?: string; field?: string }
  | { kind: "deletetask"; step: "pick" }
  | { kind: "welcome" }
  | { kind: "welcomebtn"; step: "name" | "url"; name?: string }
  | { kind: "channel" }
  | { kind: "reactchannel" }
  | { kind: "newtaskmsg" }
  | { kind: "addadmin" }
  | { kind: "removeadmin" }
  | { kind: "sendzx"; step: "who" | "amount"; who?: string }
  | { kind: "renamewelcomebtn"; step: "index" | "name"; index?: number };

const flows = (globalThis as unknown as { __zxFlows?: Map<string, Flow> }).__zxFlows ?? new Map<string, Flow>();
(globalThis as unknown as { __zxFlows: Map<string, Flow> }).__zxFlows = flows;

const announceLock = (globalThis as unknown as { __zxAnnounceAt?: number }).__zxAnnounceAt ?? 0;
(globalThis as unknown as { __zxAnnounceAt: number }).__zxAnnounceAt = announceLock;
...
function startKeyboard() {
  const url = appUrl();
  if (!url) {
    return { keyboard: [[{ text: "Open ZX Miner" }]], resize_keyboard: true };
  }
  return {
    keyboard: [[{ text: "Open ZX Miner", web_app: { url } }]],
    resize_keyboard: true,
  };
}

function welcomeMarkup(buttons: WelcomeButton[]) {
  const url = appUrl();
  const rows: Record<string, unknown>[][] = [];
  for (const b of buttons) {
    if (b.url === "webapp" || b.url === "app") {
      if (url) rows.push([{ text: b.text, web_app: { url } }]);
      else rows.push([{ text: b.text }]);
    } else if (b.url) {
      rows.push([{ text: b.text, url: b.url }]);
    }
  }
  if (!rows.length && url) {
    rows.push([{ text: "🚀 Start ZX Mining", web_app: { url } }]);
  }
  return { inline_keyboard: rows };
}

function parseRef(text: string | undefined): string | null {
  if (!text) return null;
...
function adminHelp(isOwner: boolean, users: number) {
  const lines = [
    "ZX Admin",
  ...
  if (isOwner) {
    lines.push(
      ...
      "/listadmins",
      "/sendzx <telegram_id|ZX-id> <amount>",
      "/sendatf /send_atf /credit — same as /sendzx",
    );
  }
  return lines.join("\n");
}
...
async function rateLimitedBroadcast() {
  const now = Date.now();
  const last = (globalThis as unknown as { __zxAnnounceAt: number }).__zxAnnounceAt || 0;
  if (now - last < 20_000) throw new Error("Please wait before broadcasting again.");
  (globalThis as unknown as { __zxAnnounceAt: number }).__zxAnnounceAt = now;
}

export async function broadcastPayload(payload: Record<string, unknown>) {
  ...
  await store.iterateUsers(async (user) => {
    try {
      await tg("sendMessage", { chat_id: Number(user.telegramId), ...payload });
      sent += 1;
...
export async function broadcastMessage(
  ...
  return broadcastPayload({ text, disable_web_page_preview: true, ...extra });
}

export async function broadcastPhoto(fileId: string, caption: string, extra: Record<string, unknown> = {}) {
  if (!token()) return { sent: 0, failed: 0, skipped: true };
  const store = await getStore();
  let sent = 0;
  let failed = 0;
...
export async function broadcastNewTask(task: Task) {
  const store = await getStore();
  const settings = await store.getSettings();
  const tpl = settings.newTaskBroadcastTemplate || DEFAULT_TASK_BROADCAST;
  const text = tpl
    .replaceAll("{title}", task.title)
    .replaceAll("{reward}", String(task.reward))
  ...
  if (settings.channelUrl || settings.reactChannelId) {
    try {
      const chat = settings.reactChannelId || settings.channelUrl;
      await tg("sendMessage", { chat_id: chat, text, ...extra });
    } catch {
...
async function handleAdminFlow(chatId: number, fromId: string, text: string, msg: TgMessage) {
  ...
  if (flow.kind === "announce" || flow.kind === "announcepic") {
    if (flow.kind === "announcepic" && flow.step === "photo") {
      ...
      flows.set(fromId, { ...flow, photo: file, step: "text" });
    ...
    if (flow.step === "text") {
      ...
      flows.set(fromId, { ...flow, text: body, step: "btn" });
      await send(chatId, "Button name? Send the label, or /skip for no button.", {
        reply_markup: { inline_keyboard: [[{ text: "Skip", callback_data: "flow:skip" }]] },
      });
      return true;
    }
    if (flow.step === "btn") {
      if (/^\/skip$/i.test(text) || text === "-") {
        ...
        const res = flow.photo
          ? await broadcastPhoto(flow.photo, flow.text || "")
          : await broadcastMessage(flow.text || "");
        flows.delete(fromId);
        await send(chatId, `Sent ${res.sent}, failed ${res.failed}.`);
        return true;
      }
      flows.set(fromId, { ...flow, btn: text.slice(0, 40), step: "url" });
    ...
    if (flow.step === "url") {
      ...
      await rateLimitedBroadcast();
      const isWeb = url === appUrl();
      const markup = {
        inline_keyboard: [
          isWeb ? [{ text: flow.btn, web_app: { url } }] : [{ text: flow.btn, url }],
        ],
      };
      const res = flow.photo
        ? await broadcastPhoto(flow.photo, flow.text || "", { reply_markup: markup })
        : await broadcastMessage(flow.text || "", { reply_markup: markup });
      flows.delete(fromId);
      await send(chatId, `Sent ${res.sent}, failed ${res.failed}.`);
      return true;
    }
  ...
  if (flow.kind === "addtask") {
    if (flow.step === "name") {
      flows.set(fromId, { ...flow, name: text, step: "desc" });
      await send(chatId, "Task description?");
      return true;
    }
    if (flow.step === "desc") {
      flows.set(fromId, { ...flow, desc: text, step: "link" });
      await send(chatId, "Task link? (https://… or - for none)");
      return true;
    }
    if (flow.step === "link") {
      const link = text === "-" ? "" : text;
      flows.set(fromId, { ...flow, link, step: "reward" });
  ...
  if (flow.kind === "edittask") {
    if (flow.step === "pick") {
      const tasks = await store.listTasks();
      const task = tasks.find((t) => t.id === text || t.title.toLowerCase() === text.toLowerCase());
      if (!task) {
        await send(chatId, "Task not found. Send id or exact name from /listtasks.");
        return true;
      }
      flows.set(fromId, { kind: "edittask", step: "field", taskId: task.id });
      await send(chatId, "Edit what? name / link / reward / description");
      return true;
    }
    if (flow.step === "field") {
      const field = text.toLowerCase();
      if (!["name", "link", "reward", "description", "title", "url"].includes(field)) {
        await send(chatId, "Use name, link, reward, or description.");
        return true;
      }
      flows.set(fromId, { kind: "edittask", step: "value", taskId: flow.taskId, field });
      await send(chatId, "Send the new value.");
      return true;
    }
    if (flow.step === "value" && flow.taskId) {
      const tasks = await store.listTasks();
      const task = tasks.find((t) => t.id === flow.taskId);
      if (!task) {
        flows.delete(fromId);
  ...
  if (flow.kind === "welcomebtn") {
    if (flow.step === "name") {
      flows.set(fromId, { kind: "welcomebtn", step: "url", name: text.slice(0, 40) });
      await send(chatId, "Button URL? https://… or webapp");
      return true;
    }
    const url = /^webapp$/i.test(text) ? "webapp" : text;
    const settings = await store.getSettings();
    const buttons = [...(settings.welcomeButtons || []), { text: flow.name || "Open", url }];
  ...
  if (flow.kind === "renamewelcomebtn") {
    const settings = await store.getSettings();
    const buttons = [...(settings.welcomeButtons || [])];
    if (flow.step === "index") {
      ...
      flows.set(fromId, { kind: "renamewelcomebtn", step: "name", index });
    ...
    const updated = buttons.map((b, i) => (i === flow.index ? { ...b, text: text.slice(0, 40) } : b));
  ...
  if (flow.kind === "channel") {
    await store.updateSettings({ channelUrl: text, communityUrl: text });
    flows.delete(fromId);
  ...
  if (flow.kind === "reactchannel") {
    ...
    try {
      const chat = await tg<{ id: number }>("getChat", { chat_id: `@${username}` });
      channelId = String(chat.id);
    } catch {
      if (/^-?\d+$/.test(text)) channelId = text;
    }
    const url = text.startsWith("http") ? text : `https://t.me/${username}`;
    await store.updateSettings({ reactChannelUrl: url, reactChannelId: channelId, channelUrl: url });
    const tasks = await store.listTasks();
    const react = tasks.find((t) => t.id === REACT_TASK_ID);
    if (react) await store.saveTask({ ...react, url });
    flows.delete(fromId);
    await send(
      chatId,
      channelId
        ? `React channel saved.\n${url}\nChat id ${channelId}\nMake the bot admin in that channel so reactions can be verified.`
        : `Link saved (${url}) but chat id was not resolved. Send @username after adding the bot as admin.`,
    );
    return true;
  }
  ...
  if (flow.kind === "addadmin") {
    ...
    const settings = await store.getSettings();
    const ids = Array.from(new Set([...(settings.adminTelegramIds || []), id]));
  ...
  if (flow.kind === "removeadmin") {
    ...
    await store.updateSettings({
      adminTelegramIds: (settings.adminTelegramIds || []).filter((x) => x !== id),
    });
    flows.delete(fromId);
  ...
  if (flow.kind === "sendzx") {
    ...
    if (flow.step === "who") {
      flows.set(fromId, { kind: "sendzx", step: "amount", who: text.trim() });
...
const ownerCreditLog =
  (globalThis as unknown as { __zxOwnerCredits?: Record<string, unknown>[] }).__zxOwnerCredits ?? [];
(globalThis as unknown as { __zxOwnerCredits: Record<string, unknown>[] }).__zxOwnerCredits = ownerCreditLog;

async function findCreditTarget(raw: string): Promise<User | null> {
  const id = raw.trim();
  if (!id) return null;
  const store = await getStore();
  if (/^\d+$/.test(id)) {
  ...
  const alt =
    /^zx-/i.test(id) ? `ATF-${id.slice(3)}` : /^atf-/i.test(id) ? `ZX-${id.slice(4)}` : "";
  const queries = alt ? [id, alt] : [id];
  const seen = new Set<string>();
  const pool: User[] = [];
  for (const q of queries) {
    const { items } = await store.listUsers({ q, limit: 25 });
  ...
  return (
    pool.find(
      (u) =>
        u.atfId.toLowerCase() === needle ||
        (altNeedle && u.atfId.toLowerCase() === altNeedle) ||
        u.telegramId === id ||
        u.id === id ||
...
async function creditZxFromOwner(chatId: number, ownerId: string, whoRaw: string, amountRaw: string) {
  if (!isOwnerId(ownerId)) {
    await send(chatId, "Owner only.");
    return;
  }
  try {
    rateLimit(`sendzx:${ownerId}`, 8, 60_000);
  } catch (err) {
  ...
  try {
    await notifyUser(
      next.telegramId,
      `The owner credited ${credited} ZX to your Pool Wallet.\nNew Pool balance: ${next.balance.toFixed(4)} ZX`,
    );
  } catch {
    /* optional */
  }
...
async function handleAdminCommand(chatId: number, fromId: string, text: string) {
  const store = await getStore();
  const c = cmd(text);
  const rest = arg(text);

  if (c === "/announcement") {
    flows.set(fromId, { kind: "announce", step: "text" });
    await send(chatId, "Send the announcement text.");
    return;
  }
  if (c === "/announcepic" || c === "/announce_image") {
    flows.set(fromId, { kind: "announcepic", step: "photo" });
    await send(chatId, "Send the image.");
    return;
  }
  if (c === "/addtask") {
    flows.set(fromId, { kind: "addtask", step: "name" });
    await send(chatId, "Task name?");
    return;
  }
  if (c === "/edittask") {
    flows.set(fromId, { kind: "edittask", step: "pick" });
    const tasks = await store.listTasks();
    const lines = tasks.map((t) => `${t.id} — ${t.title} (+${t.reward})`).join("\n") || "No tasks.";
    await send(chatId, `Send task id or name to edit:\n${lines}`);
    return;
  }
  if (c === "/deletetask") {
    flows.set(fromId, { kind: "deletetask", step: "pick" });
    const tasks = await store.listTasks();
    const lines = tasks.map((t) => `${t.id} — ${t.title}`).join("\n") || "No tasks.";
  ...
  if (c === "/setreactchannel") {
    if (rest) {
      ...
      await handleAdminFlow(chatId, fromId, rest, { chat: { id: chatId }, text: rest });
      return;
    }
    flows.set(fromId, { kind: "reactchannel" });
  ...
  if (c === "/listwelcomebuttons") {
    const s = await store.getSettings();
    const lines = (s.welcomeButtons || []).map((b, i) => `${i + 1}. ${b.text} → ${b.url}`).join("\n") || "None.";
    await send(chatId, lines);
    return;
  }
  if (c === "/setchannel" || c === "/setcommunity") {
    if (rest) {
      await store.updateSettings({ channelUrl: rest, communityUrl: rest });
  ...
  if (c === "/listadmins") {
    ...
    const s = await store.getSettings();
    const owner = process.env.OWNER_TELEGRAM_ID || "(not set)";
    const envA = (process.env.ADMIN_TELEGRAM_IDS || "").trim() || "(none)";
    const dbA = (s.adminTelegramIds || []).join(", ") || "(none)";
    await send(chatId, `Owner: ${owner}\nEnv admins: ${envA}\nDB admins: ${dbA}`);
    return;
  }
  if (c === "/sendzx" || c === "/sendatf" || c === "/credit" || c === "/send_atf") {
    ...
    if (parts.length >= 2) {
      ...
      await creditZxFromOwner(chatId, fromId, who, amountToken);
      return;
    }
    if (parts.length === 1) {
      flows.set(fromId, { kind: "sendzx", step: "amount", who: parts[0] });
      await send(chatId, "Amount of ZX to credit to Pool Wallet?");
      return;
    }
    flows.set(fromId, { kind: "sendzx", step: "who" });
...
export async function handleTelegramUpdate(update: TgUpdate) {
  if (update.channel_post) {
    ...
    if (!settings.reactChannelId || settings.reactChannelId === chatId) {
      ...
      if (!settings.reactChannelId) {
        await store.updateSettings({ reactChannelId: chatId });
      }
    }
    return;
  }
  ...
  if (update.callback_query) {
    ...
    if (q.data === "flow:skip" && chatId) {
      const flow = flows.get(fromId);
      await answerCb(q.id);
      if (flow && (flow.kind === "announce" || flow.kind === "announcepic") && flow.step === "btn") {
        try {
          await rateLimitedBroadcast();
          const res = flow.photo
            ? await broadcastPhoto(flow.photo, flow.text || "")
  ...
  if (c === "/start" || text === "Open ZX Miner" || text === "Open ATF Miner") {
    const existing = await store.getUserByTelegramId(telegramId);
    if (!existing) {
  ...
  if (c === "/help") {
    const admin = await isAdminId(telegramId);
    if (!admin) {
      await send(msg.chat.id, USER_HELP, { reply_markup: startKeyboard() });
      return;
    }
    const stats = await store.stats();
    await send(msg.chat.id, adminHelp(isOwnerId(telegramId), stats.users), {
      reply_markup: startKeyboard(),
    });
    return;
  }
...
export async function ensureTelegramWebhook() {
  const t = token();
  const base = webappUrl();
  if (!t || !base) return { ok: false, reason: "BOT_TOKEN or WEBAPP_URL missing" };
  ...
  const menuUrl = `${base}/app`;
  return { ok: true, url, menuUrl };
}

export async function notifyUser(telegramId: string, text: string) {
  if (!token()) return;
  try {
    await tg("sendMessage", { chat_id: Number(telegramId), text });
  } catch (err) {
